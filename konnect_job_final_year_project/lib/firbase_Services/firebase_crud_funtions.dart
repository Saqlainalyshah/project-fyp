...
class FirebaseUserServices{
  ...
  Future<String> registerUser(String userId ,UserModelClass userModelClass) async {
    try {
...
/// add subcollection
  ...
  static Stream<QuerySnapshot<Map<String, dynamic>>> getAllUsersData(String userId){
...
class FirebaseWorkersServices{
  ...
  final CollectionReference _workersCollection = FirebaseFirestore.instance.collection('Workers');

  Future<String> registerWorker(String userId,WorkerSkillsDataModelClass workerSkillsDataModelClass) async {
    try {
      // Assuming userId is part of the UserModelClass
  ...
  Future<void> updateWorker(String workerId, Map<String, dynamic> updatedData) async {
    try {
  ...
  Future<List<UserModelClass>> getAllWorkers() async {
    try {
      ...
      snapshot.docs.forEach((doc) {
        workers.add(UserModelClass.fromJSON(doc.data() as Map<String, dynamic>));
      });
      return workers;
    } catch (e) {
...
class FirebaseJobsServices {
  ...
  Future<void> createJob(JobModelClass jobModelClass) async {
    try {
      ...
      DocumentReference docRef = await _jobsCollection.add(
          jobModelClass.toJSON());
    } catch (e) {
  ...
  Future<void> updateJobDetails(String jobId,
...
class FirebaseSavedJobService{
  final CollectionReference _savedJobCollection = FirebaseFirestore.instance.collection('SavedJobs');

  Future<void> savedJob(SavedJobsModelClass savedJobsModelClass) async {
    try {
      // Upload job document to jobs collection
      DocumentReference docRef = await _savedJobCollection.add(savedJobsModelClass.toJSON());
    } catch (e) {
      print("Error uploading job details: $e");
      throw e; // Rethrow the error for handling in the UI
    }
  ...
  Future<List<SavedJobsModelClass>> getSavedJobs() async {
    ...
    try {
      ...
      querySnapshot.docs.forEach((doc) {
        ...
        SavedJobsModelClass savedJob = SavedJobsModelClass.fromJSON(data as Map<String,dynamic>);
  ...
  Future<void> addToSavedJobs(String curentUserId,JobModelClass job, UserModelClass user) async {
    try {
...
class ApplicantsService {
  ...
  final CollectionReference _applicantsCollection = FirebaseFirestore.instance.collection('applicants');
  ...
  Stream<List<ApplicantsModel>> getApplicants() {
    return _applicantsCollection.snapshots().map((snapshot) =>
        snapshot.docs.map((doc) => ApplicantsModel.fromJSON(doc.data() as Map<String, dynamic>)).toList());
  }

  // Update operation
  Future<void> updateApplicant(ApplicantsModel applicant) async {
    try {
      await _applicantsCollection.doc(applicant.applicantId).update(applicant.toJSON());
    } catch (e) {
